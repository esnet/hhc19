...
function crack(flags){
    ...
    flag_index = flags.indexOf(getTestFlag(uuid));
    if( flag_index >= 0 )
	console.log("Found seed for", flags[flag_index], uuid);

    
    while(flags){
	flag_index = flags.indexOf(getTestFlag(uuid + "_" + toRadix(index)));
        if( flag_index >= 0 )
	    console.log("Found seed for", flags[flag_index], uuid + "_" + toRadix(index));
